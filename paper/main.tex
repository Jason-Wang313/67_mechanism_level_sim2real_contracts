        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Mechanism-Level Sim2Real Contracts: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study sim-to-real transfer for robot manipulation. The submission-hardened claim is intentionally narrow: Transfer by preserving causal mechanisms rather than matching observation statistics. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Mechanism level sim2real contracts should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.412 $\pm$ 0.012 & 0.456 $\pm$ 0.014 & 0.334 $\pm$ 0.011 \\
Data-augmented WAM & 0.492 $\pm$ 0.018 & 0.530 $\pm$ 0.015 & 0.425 $\pm$ 0.011 \\
Scalar uncertainty & 0.551 $\pm$ 0.016 & 0.576 $\pm$ 0.013 & 0.499 $\pm$ 0.013 \\
Ensemble disagreement & 0.588 $\pm$ 0.016 & 0.627 $\pm$ 0.015 & 0.551 $\pm$ 0.019 \\
Conformal risk filter & 0.622 $\pm$ 0.016 & 0.638 $\pm$ 0.014 & 0.574 $\pm$ 0.011 \\
Proposed branch mechanism & 0.691 $\pm$ 0.013 & 0.710 $\pm$ 0.011 & 0.662 $\pm$ 0.012 \\
Oracle branch upper bound & 0.762 $\pm$ 0.018 & 0.776 $\pm$ 0.009 & 0.743 $\pm$ 0.015 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.672 $\pm$ 0.010 & all components \\
Minus branch atlas & 0.565 $\pm$ 0.013 & removes explicit unrealized futures \\
Minus tail risk & 0.578 $\pm$ 0.015 & uses mean predicted value only \\
Minus diagnostic probe & 0.610 $\pm$ 0.019 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.607 $\pm$ 0.020 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/67.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Mechanism-Level Sim2Real Contracts is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
