        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Mechanism-Level Sim2Real Contracts: Branch-Structured Evidence for Embodied World-Action Models}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study sim-to-real transfer for robot manipulation. The motivating bet is: Transfer by preserving causal mechanisms rather than matching observation statistics.. We introduce a mechanism-level representation that keeps physically admissible but unrealized action outcomes as explicit branches. A synthetic contact-style evaluation shows that the proposed branch mechanism improves hidden-mode success and tail-risk relative to observed-only, augmentation-as-data, and scalar-uncertainty baselines.
        \end{abstract}

        \section{Introduction}
        Robotics papers often collapse unobserved physical alternatives into extra data, variance, or a single predicted next state. For sim-to-real transfer for robot manipulation, this can erase the difference between modes that are visually similar but action-critical. This paper asks whether a world-action model should expose those alternatives to the planner before execution.
...
augmentation\_as\_data\_wam & nominal & 0.575 & 0.425 \\
augmentation\_as\_data\_wam & hidden\_mode\_shift & 0.255 & 0.745 \\
scalar\_uncertainty\_wam & nominal & 0.625 & 0.375 \\
scalar\_uncertainty\_wam & hidden\_mode\_shift & 0.362 & 0.637 \\
...
\end{tabular}
        ...
        \section{Conclusion}
        Mechanism-Level Sim2Real Contracts argues that embodied world-action models should preserve unrealized physical alternatives when those alternatives change downstream action value.
